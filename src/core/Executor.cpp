...
Value Executor::eval_expr(parser::Expression* expr, const storage::Record& rec, const parser::TableMetadata& meta) {
    ...
    if (auto col = dynamic_cast<parser::ColumnExpr*>(expr)) {
        ...
        if (meta.columns[idx].type == "int") {
            return {true, rec.values[idx], ""};
        } else {
            return {false, 0, catalog_.getStringPool().get_string(rec.values[idx])};
        }
    }
...
bool Executor::eval_cond(parser::Expression* expr, const storage::Record& rec, const parser::TableMetadata& meta) {
    ...
    if (auto bin = dynamic_cast<parser::BinaryExpr*>(expr)) {
        if (bin->op.value == "and") {
            return eval_cond(bin->left.get(), rec, meta) && eval_cond(bin->right.get(), rec, meta);
        }
        if (bin->op.value == "or") {
            return eval_cond(bin->left.get(), rec, meta) || eval_cond(bin->right.get(), rec, meta);
        }
    ...
    if (auto bet = dynamic_cast<parser::BetweenExpr*>(expr)) {
        ...
        if (val.is_int) {
            return val.int_val >= low.int_val && val.int_val < high.int_val;
        } else {
            return val.str_val >= low.str_val && val.str_val < high.str_val;
        }
    }
    return false;
}
...
void Executor::execute(const parser::Statement& stmt) {
    switch (stmt.type) {
        case parser::StatementType::CREATE_DATABASE:
            execute_create_database(static_cast<const parser::CreateDatabaseStatement&>(stmt)); break;
        case parser::StatementType::DROP_DATABASE:
            execute_drop_database(static_cast<const parser::DropDatabaseStatement&>(stmt)); break;
        case parser::StatementType::USE:
            execute_use(static_cast<const parser::UseStatement&>(stmt)); break;
        case parser::StatementType::CREATE_TABLE:
            execute_create_table(static_cast<const parser::CreateTableStatement&>(stmt)); break;
        case parser::StatementType::DROP_TABLE:
            execute_drop_table(static_cast<const parser::DropTableStatement&>(stmt)); break;
        case parser::StatementType::INSERT:
            execute_insert(static_cast<const parser::InsertStatement&>(stmt)); break;
        case parser::StatementType::UPDATE:
            execute_update(static_cast<const parser::UpdateStatement&>(stmt)); break;
        case parser::StatementType::DELETE:
            execute_delete(static_cast<const parser::DeleteStatement&>(stmt)); break;
        case parser::StatementType::SELECT:
            execute_select(static_cast<const parser::SelectStatement&>(stmt)); break;
        default: break;
    }
}

void Executor::execute_create_database(const parser::CreateDatabaseStatement& stmt) {
    catalog_.createDatabase(stmt.db_name);
}

void Executor::execute_drop_database(const parser::DropDatabaseStatement& stmt) {
    catalog_.dropDatabase(stmt.db_name);
}

void Executor::execute_use(const parser::UseStatement& stmt) {
    catalog_.setActiveDatabase(stmt.db_name);
}
...
void Executor::execute_drop_table(const parser::DropTableStatement& stmt) {
    catalog_.dropTable(catalog_.getActiveDatabase(), stmt.table_name);
}
    ...
    for (const auto& row_vals : stmt.values) {
        ...
        for (size_t i = 0; i < meta.columns.size(); ++i) {
            ...
            if (stmt.columns.empty()) {
                if (i < row_vals.size()) {
                    provided = true;
                    val_str = row_vals[i].value;
                }
            } else {
                for (size_t j = 0; j < stmt.columns.size(); ++j) {
                    if (stmt.columns[j] == colDef.name) {
                        provided = true;
                        val_str = row_vals[j].value;
                        break;
                    }
                }
            }
            ...
            if (!provided) {
                if (!colDef.default_value.empty()) {
                    val_str = colDef.default_value;
                } else if (colDef.is_not_null) {
                    throw std::runtime_error("");
                }
            }

            if (colDef.type == "int") {
                rec.values[i] = val_str.empty() ? 0 : std::stoi(val_str);
            } else {
                rec.values[i] = val_str.empty() ? 0 : catalog_.getStringPool().get_or_add_string(val_str);
            }
        }
...
void Executor::execute_update(const parser::UpdateStatement& stmt) {
    ...
    for (uint32_t row_id = 0; row_id < all_recs.size(); ++row_id) {
        ...
        if (eval_cond(stmt.where_clause.get(), r, meta)) {
            ...
            uint32_t old_idx_val = (idx_col != -1) ? r.values[idx_col] : 0;

            for (const auto& assign : stmt.assignments) {
                for (size_t i = 0; i < meta.columns.size(); ++i) {
                    if (meta.columns[i].name == assign.first) {
                        if (meta.columns[i].type == "int") r.values[i] = std::stoi(assign.second.value);
                        else r.values[i] = catalog_.getStringPool().get_or_add_string(assign.second.value);
                        break;
                    }
                }
            }

            if (idx_col != -1 && old_idx_val != r.values[idx_col]) {
                table.remove_from_index(old_idx_val, row_id);
                table.add_to_index(r.values[idx_col], row_id);
            }
...
void Executor::execute_delete(const parser::DeleteStatement& stmt) {
    ...
    for (uint32_t row_id = 0; row_id < all_recs.size(); ++row_id) {
        ...
        if (eval_cond(stmt.where_clause.get(), r, meta)) {
            int idx_col = table.get_indexed_col();
            if (idx_col != -1) {
                table.remove_from_index(r.values[idx_col], row_id);
            }
...
void Executor::execute_select(const parser::SelectStatement& stmt) {
            ...
            if (bin->op.value == "==") {
                ...
                if (idx_col != -1 && col && lit && meta.columns[idx_col].name == col->column_name) {
                    uint32_t search_val = (lit->value.type == parser::TokenType::NUMBER) ?
                        std::stoi(lit->value.value) :
                        catalog_.getStringPool().get_or_add_string(lit->value.value);

                    storage::Record found_rec;
                    if (table.find_by_id(search_val, found_rec) && !found_rec.is_deleted) {
    ...
    if (stmt.select_all) {
        for (const auto& c : meta.columns) headers.push_back(c.name);
    } else {
        for (const auto& c : stmt.columns) headers.push_back(c.alias.empty() ? c.name : c.alias);
    }
